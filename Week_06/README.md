...
先上week6，参见 [week6学习笔记](study-notes.md) （思维导图图片格式），或是直接打开本文件夹下的 java相关框架.xmind   性能与SQL优化.xmind  两个文件即可。

### 挑战作业

基于 AOP 和自定义注解，实现 @MyCache(60) 对于指定方法返回值缓存 60 秒。

参见文件夹内的工程customized-aop-mycache

基本思路：使用延迟队列保存<key, 缓存时间>, 另外单独启动一个定时扫描任务，隔一段时间从延迟队列拿出过期的数据并从本地缓存中删除。
...
说明：从业务、以及数据库查询优化角度（值为NULL的字段不会走索引）考虑，一般字段应尽量设置成NOT NULL、给一个有意义的默认值，但是吧，挨个检查的话有点麻烦，于是跳过这步、直接用NULL了~.~
